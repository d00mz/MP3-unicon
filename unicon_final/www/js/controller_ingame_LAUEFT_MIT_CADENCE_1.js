...
app.controller('IngameCtrl', function($scope, $http, geolocation, $window, Auth) {
	...
	$scope.init = function(){
        ...
        }, ($scope.sound.duration / $scope.sound.cadence)-200);
	};
	...
	$scope.jamHandler = function(){
		...
		if($scope.shakeCounter.loopFinished == 1) {	// loop beendet - neuen Loop anfangen
			for(var i = 0; i < $scope.ingame.instruments.length; i++){
					...
					$('#'+ $scope.ingame.instruments[i].id + ' h2').html('S ' + $scope.ingame.instruments[i].playState.stage + ' | v ' + $scope.ingame.instruments[i].playState.volume);


					//$scope.play(inst, index, volume);
					$scope.playAtTime(inst, indexOld, indexNew, volume);
			}

		} else {	// Takt 1,2,3 - Crossfade/Fadeout nötig?
			for(var i = 0; i < $scope.ingame.instruments.length; i++){
					console.log($scope.ingame.instruments[i].playState);
	...
	*/
	$scope.wsConnect = function(){
		...
		socket.on('updateInstrument',function(data) {
			console.log('New Instrument Data!', data);
			for(var i = 0; i < $scope.ingame.instruments.length; i++){
	...
	$scope.playAtTime = function(instrument, audioOut, audioIn, value){
		...
		console.log('('+audioOut +'!='+ audioIn +'&&'+ $scope.shakeCounter.loopFinished +'<'+ $scope.sound.cadence+') || '+$scope.sound.cadence +'== 1)');

		if((audioOut != audioIn && $scope.shakeCounter.loopFinished < $scope.sound.cadence) || $scope.sound.cadence == 1){
			console.log('start at: '+ ($scope.sound.duration/$scope.sound.cadence) * ($scope.shakeCounter.loopFinished-1));

			// alten Track pausieren
			$audioOut[0].pause();

			// neuen Track an passendem Takt abspielen
			$audioIn[0].currentTime = ($scope.sound.duration/$scope.sound.cadence) * ($scope.shakeCounter.loopFinished-1);
			$audioIn[0].volume = value;
