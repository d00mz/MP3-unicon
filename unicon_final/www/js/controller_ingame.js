...
app.controller('IngameCtrl', function($scope, $http, geolocation, $window, Auth) {
	...
	$scope.init = function(){
        ...
        }, ($scope.sound.duration / $scope.sound.cadence)-150);
	};
	...
	$scope.jamHandler = function(){
		...
		for(var i = 0; i < $scope.ingame.instruments.length; i++){
				...
				console.log($scope.ingame.instruments[i].instrumentname, $scope.ingame.instruments[i].playState);
	...
	*/
	...
	$scope.playAtTime = function(instrument, audioOut, audioIn, value){
		...
		if(audioIn != 0){
			console.log('allla wakbar');

			// alten Track pausieren
			//$audioOut[0].pause();
			$('.' + instrument + ' > audio:eq(0)')[0].pause();
			$('.' + instrument + ' > audio:eq(1)')[0].pause();
			$('.' + instrument + ' > audio:eq(2)')[0].pause();
